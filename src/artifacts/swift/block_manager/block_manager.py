...
class BlockManagerArgs:
    ...
    @classmethod
    def init_new(cls, device_name, num_blocks, args: EngineArgs) -> 'BlockManagerArgs':
        return cls(
...
class BlockManagerArtifact(Artifact):
    """
    ...
    def __init__(self, args: BlockManagerArgs):
        self.device_name = args.device_name
        self.num_free_blocks = args.num_blocks
        ...
        self.block_table = torch.empty(
            (args.max_seqs_in_block_table, args.max_blocks_per_seq),
            dtype=torch.int32,
            device="cuda",
        )
    ...
    def register(self, service: BaseService):
        ...
        for method in methods_to_register:
            self._register_method(method, service, artifact_name='block_manager')
