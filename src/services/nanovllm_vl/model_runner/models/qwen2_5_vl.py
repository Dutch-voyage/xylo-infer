...
from ..layers.vision_attention import VisionAttention
...
from .utils import permute_inv

from src.services.nanovllm_vl.engine.mm_io_struct import ImageDataItem
from src.services.nanovllm_vl.model_runner.models.mm_utils import ImageInputs
from src.services.nanovllm_vl.utils.context import get_context_field
...
class Qwen2_5_VLForConditionalGeneration(nn.Module, Artifact):    
    ...
    def pad_input_ids(
        ...
        """
        if not input_ids or not image_inputs.image_items:
            return input_ids
        ...
        for item in image_inputs.image_items:
            if item is not None and image_inputs.im_token_id is not None:
                token_to_pad_mapping[image_inputs.im_token_id] = item.pad_value
            else:
                raise ValueError(f"No multimodal token id provided for image item: {item}")
    ...
    def forward(
        ...
        image_inputs = get_context_field("image_inputs")
    ...
    def get_image_feature(self, items: List[ImageDataItem]) -> torch.Tensor:
